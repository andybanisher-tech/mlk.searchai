...
if (check_bitrix_sessid() && isset($_REQUEST['generate']) && $_REQUEST['generate'] === 'Y') {
    ...
    while ($el = $res->Fetch()) {
        ...
        // Попробуем сгенерировать эмбеддинг
    ...
    CAdminMessage::ShowMessage(['TYPE' => 'OK', 'MESSAGE' => Loc::getMessage('MLK_SEARCHAI_EMBEDDINGS_GENERATED', ['#COUNT#' => $count])]);
}
...
<form method="post" action="<?= $APPLICATION->GetCurPage() ?>?lang=<?= LANGUAGE_ID ?>">
    ...
    <input type="submit" value="<?= Loc::getMessage('MLK_SEARCHAI_EMBEDDINGS_START') ?>">
</form>
